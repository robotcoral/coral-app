...
export class World extends Group {
  ...
  private raiseFlag(coo: Coordinates2) {
    this.flags[coo.x][coo.y].position.add(
      new Vector3(0, this.gridScale / 2, 0)
    );
  }
  ...
  placeFlag(coo: Coordinates2, color = COLORS.RED) {
    ...
    if (this.isStackMinHeight(coo, 1))
      this.flags[coo.x][coo.y].position.addScaledVector(
        new Vector3(0, (this.objects[coo.x][coo.y] as []).length / 2, 0),
        this.gridScale
      );
  ...
  isFlag(coo: Coordinates2, color?: COLORS) {
    return (
      this.flags[coo.x][coo.y] &&
      (color ? this.flags[coo.x][coo.y].color == color : true)
    );
  }
